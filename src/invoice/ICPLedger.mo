...
module {
    ...
    public class Factory(id : Principal) = {

    let Ledger_actor : Ledger.Interface = actor(Principal.toText(id));
    ...
    public func transfer (args : TransferArgs) : async TransferResult {
      let result = await Ledger_actor.transfer(args);
      switch result {
        case (#Ok index) {
    ...
    public func balance(args : AccountArgs) : async BalanceResult {
      switch (Hex.decode(args.account)) {
        ...
        case (#ok account) {
          let balance = await Ledger_actor.account_balance({account = Blob.fromArray(account)});
          #ok({
