  ...
  <div class="flex lg:flex-row flex-col min-h-screen justify-around" v-else>
    ...
    <div class="flex flex-col lg:w-1/2">
      ...
      <div class="flex flex-col items-center">
        <div class="flex lg:flex-row flex-wrap flex-col justify-around gap-x-6 gap-y-8 mt-12 px-6">
          <li v-for="type in list_types_accessories" :key="type.name" class="list-none">
            <button class="md:text-2xl shadow-xl text-gray-800 font-bold bg-pink-400 rounded px-8 py-4 w-full lg:px-14 lg:py-6 lg:w-auto" :class="[selectedType.name == type.name ? 'bg-pink-600' : '']" @click="changeSelectedType(type)">
              {{ type.name }}
            </button>
          </li>
        </div>
        <h2 class="lg:w-2/3 px-6 lg:mt-16 mt-8 mx-auto text-center text-gray-800 text-2xl font-bold">
          {{ selectedType.message }}
        </h2>
        <div class="flex flex-row flex-wrap justify-center mt-24 gap-x-4 gap-y-6" v-if="accessoryList.length > 0 && conflictSlots.length == 0">
          <li v-for="accessory in accessoryList" :key="accessory.name" class="list-none flex flex-row flex-wrap cursor-pointer" @click="changeSelectedAccessory(accessory)" :class="[selectedAccessory != undefined && selectedAccessory.name === accessory.name ? 'border-4 border-pink-400' : 'border-4 border-gray-400']">
            <svg viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg" width="200px">
              <use :href="`#${accessory.name}`"></use>
            </svg>
          </li>
        </div>
        <div v-else-if="conflictSlots.length > 0" class="flex flex-row flex-wrap justify-center mt-24 gap-x-4 gap-y-6">
      ...
      <div v-if="accessoryList.length > 0 && conflictSlots.length == 0">
        <h2 class="py-12 text-center text-gray-800 text-3xl font-bold">
          {{ selectedAccessory == undefined ? "Select an accessory to check how it fits you 😎" : selectedAccessory.description }}
        </h2>
      </div>
      <div v-if="ownAccessory" class="mx-auto">
        <button class="md:text-2xl shadow-xl text-gray-800 font-bold bg-pink-600 rounded px-8 py-4 w-full lg:px-14 lg:py-6 lg:w-auto" :class="[waiting ? 'animate-pulse cursor-wait' : '']" @click="wearAccessory(selectedAccessory.name)">
          {{ waiting ? "Wait.. " : "Wear 👈" }}
        </button>
      </div>
    </div>
...
import { isAccessoryInInventory, getTokenIdentifier } from "../../types/inventory";
  ...
  setup() {
    ...
    const accessoryList = computed(() => {
      ...
      return accessories.filter((accessory) => {
        return slotsEqual([accessory.slot], selectedType.value.slots);
      });
    });
    ...
    const ownAccessory = computed(() => {
      ...
      return isAccessoryInInventory(selectedAccessory.value.name, inventory.value);
    });

    const wearAccessory = async (accessory_name: string) => {
      if (waiting.value) return;
      if (confirm(`You are about to wear  ${accessory_name}. It will equip the corresponding slot(s). Are you sure? This action cannot be undone !`)) {
        waiting.value = true;
        let token_accessory = getTokenIdentifier(accessory_name, inventory.value);
        let token_avatar = store.getters.getTokenAvatar;
        let actor = store.getters.getAuthenticatedActor_material;
        if (token_avatar == null || token_accessory == "Null" || actor == null) {
          waiting.value = false;
          throw new Error("Avatar token / accessory token / actor -> not found");
        }
    ...
    return {
      ...
      loaded: computed(() => store.getters.isAvatarPreviewLoaded),
