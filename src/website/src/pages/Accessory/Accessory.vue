<template>
  <div>
    <div v-if="!list">
      ...
      <button
        class="bg-black rounded-2xl flex flex-col items-center justify-center md:text-4xl text-sm font-marker text-white cursor-poin md:h-24 h-16 accessory-item lg:w-1/6 w-1/3 mx-auto mt-24"
        @click="showList"
      >
        Try
      </button>
    ...
    <div v-else class="flex flex-row flex-wrap w-full gap-y-28 gap-x-8">
      ...
      <li class="list-none flex flex-col w-1/2 md:w-1/4 mx-25 border-2 border-gray-300" v-for="accessory in accessories" :key="accessory[0]">
        <img :src="`/accessory/${accessory[0]}.jpg`" alt="Error" />
        <card-blueprint :blueprint="accessory[1].blueprint"></card-blueprint>
        <button class="lg:text-3xl md:text-2xl text-xl shadow-xl mb-4 font-marker text-gray-800 bg-pink-600 rounded py-6 px-8 mt-8 w-1/2 mx-auto cursor-pointer" @click="createAccessory(accessory[0])">
          Create 🧪
        </button>
      </li>
    </div>
...
import { getAllAccessory, AccessoryInfos, requestCreateAccessory } from "../../api/accessory";
