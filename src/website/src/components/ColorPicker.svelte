<script lang="ts">
  ...
  $: selectedColorHex = rgbToHex(selectedColorRGB[0], selectedColorRGB[1], selectedColorRGB[2]);
...
<div class="color-picker">
  <div class="preview" on:click={() => (showDetails = !showDetails)}>
    <div class="left">
      <div style="background-color: {selectedColorHex}" class="currentColorSquare" />
  ...
  <div class="details {showDetails ? 'showing' : ''}">
    ...
    {#each suggestedColors[componentName] as c}
      <div class="suggested-color" style="background-color: rgb({c[0] + ',' + c[1] + ',' + c[2]})" on:click={() => updateAvatarColor(componentName, { r: c[0], g: c[1], b: c[2] })} />
    {/each}
    <label>
      <input on:input={handleColorChange} name={componentName} type="color" value={selectedColorHex} />
