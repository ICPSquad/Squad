...
module {
  ...
  public class Factory(dependencies : Types.Dependencies) {
    ...
    public func purgeStyleScores(t1 : Time.Time, t2 : Time.Time) : async Result.Result<Nat, Text> {
      let dates = _getDatesBetween(t1, t2);
      var total = 0;
      let tokenIds = await (AVATAR_ACTOR.tokens_ids());
      for (date in dates.vals()) {
        for (tokenId in tokenIds.vals()) {
          switch (style_score_daily.remove((date, tokenId))) {
            case (null) {};
            case (?some) {
              total += 1;
            };
