...
module {
  ...
  public class Factory(dependencies : Types.Dependencies) {
    ...
    public func startRound() : async Result.Result<Nat, Text> {
      ...
      var l : Buffer.Buffer<(Principal, ?Name, ?TokenIdentifier, ?StyleScore, ?EngagementScore, TotalScore)> = Buffer.Buffer<
    ...
    public func getRound(n : Nat) : ?Round {
    ...
    public func getBestHolders() : async [(AccountIdentifier, Nat, ?Principal, ?Text, ?Text, ?TokenIdentifier)] {
      ...
      let infos = Array.map<(Principal, ?AccountIdentifier, ?Text, ?Text, ?TokenIdentifier), (Principal, AccountIdentifier, Text, Text, TokenIdentifier)>(
        infos_opt,
        func(x) {
          (
      ...
      return (
        Array.sort<(AccountIdentifier, Nat, ?Principal, ?Text, ?Text, ?TokenIdentifier)>(
          r.toArray(),
          func(x, y) {
